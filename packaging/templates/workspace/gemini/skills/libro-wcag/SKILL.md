---
name: libro-wcag
description: Vendor-neutral WCAG workflow for creating new web pages or remediating existing pages with selectable WCAG model (2.0/2.1/2.2 and A/AA/AAA). Use when Gemini workspace automation must perform consistent accessibility audit/remediation, produce aligned Markdown+JSON outputs, and attach W3C official citations.
---

# Libro.AgentWCAG

This workspace skill mirrors the canonical `skills/libro-wcag/SKILL.md` contract for Gemini discovery.

Gemini-specific note:

- Prefer `skills/libro-wcag/adapters/gemini/prompt-template.md` when wiring custom instructions or wrapper prompts.

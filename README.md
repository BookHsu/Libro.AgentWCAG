...
- `Claude`：先用 `python .\scripts\libro.py install claude`；如果是直接開 repo，也可用 [SKILL.md](/c:/Source/Libro.AgentWCAG.clean/.claude/skills/libro-wcag/SKILL.md) 或 Claude plugin
- `Gemini`：先用 `python .\scripts\libro.py install gemini`；如果是直接開 repo，也可用 [SKILL.md](/c:/Source/Libro.AgentWCAG.clean/.gemini/skills/libro-wcag/SKILL.md)
- `Copilot`：先用 `python .\scripts\libro.py install copilot`；若要 workspace MCP，搭配 `--workspace-root` 與 `--emit-mcp-config`

### Claude Repo-Open 與 Plugin

- clone repo 後，Claude 可從 [SKILL.md](/c:/Source/Libro.AgentWCAG.clean/.claude/skills/libro-wcag/SKILL.md) 發現 `libro-wcag`
- 如需 plugin 路徑：
...
`mcp-server/server.py` 提供單一 `libro-wcag` MCP server，可同時服務 Claude、Copilot、Gemini。

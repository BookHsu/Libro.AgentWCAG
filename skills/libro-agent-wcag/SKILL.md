...
Use `scripts/auto_fix.py` for safe first-pass local HTML rewrites. It currently supports a focused set of rules such as missing `lang`, `xml:lang` mismatches, invalid language-of-parts values, missing `alt` text on images, image inputs, and image map areas, missing button names, missing link names, ARIA widget names, simple form control labels, document titles, automatic meta refresh removal, and unsafe viewport settings, and emits a unified diff for verification.

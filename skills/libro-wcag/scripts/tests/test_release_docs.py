...
class ReleaseDocsTests(unittest.TestCase):
    ...
    def test_release_docs_cover_release_readiness_and_adoption_paths(self) -> None:
        playbook = (self.repo_root / 'docs' / 'release' / 'release-playbook.md').read_text(encoding='utf-8')
        smoke = (self.repo_root / 'docs' / 'release' / 'adoption-smoke-guide.md').read_text(encoding='utf-8')
        workflow = (self.repo_root / 'docs' / 'release' / 'ga-release-workflow.md').read_text(encoding='utf-8')
        ...
        self.assertIn('vX.Y.Z', playbook)
        self.assertIn('vX.Y.Z-rc.N', playbook)
        self.assertIn('source_revision', playbook)
        self.assertIn('package-release.py', playbook)
        self.assertIn('libro-wcag-<version>-all-in-one.zip', playbook)
        self.assertIn('libro-wcag-<version>-sha256sums.txt', playbook)
        self.assertIn('install-latest.ps1', smoke)
        self.assertIn('run-release-adoption-smoke.py', smoke)
        self.assertIn('doctor-agent.py --verify-manifest-integrity', smoke)
        self.assertIn('Post-Publish Verification', workflow)
    ...
    def test_release_playbook_documents_version_and_provenance_sources(self) -> None:
        ...
        self.assertIn('npm non-latest dist-tag', content)
    ...
    def test_ga_and_rollback_docs_define_operator_contracts(self) -> None:
        ...
        self.assertIn('vX.Y.Z-rc.N', ga_definition)
        self.assertIn('validate', ga_workflow)
        self.assertIn('clean-release-smoke', ga_workflow)
        self.assertIn('GitHub pre-release', ga_workflow)
        self.assertIn('npm dist-tag', ga_workflow)
        ...
        self.assertIn('publish-npm.yml', ga_workflow)
        self.assertIn('GitHub auto-generated release notes', ga_workflow)
